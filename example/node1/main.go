...
func (s *Service1) OnInit() error {
	...
	s.AfterFunc(time.Second*2, "method test demo", func(timer *timingwheel.Timer, args ...interface{}) {
		...
		defer cancel()
		if err := s.SelectSameServer("", "Service2").Call(ctxWithTimeout, "APITest2", nil, nil); err != nil {
	...
	s.AfterFunc(time.Second*2, "method test demo1", func(timer *timingwheel.Timer, args ...interface{}) {
		...
		var out int
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APISum", []interface{}{1, 2}, &out); err != nil {
	...
	s.AfterFunc(time.Second*3, "method test demo2", func(timer *timingwheel.Timer, args ...interface{}) {
		...
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIPrintParams", []interface{}{1, "2"}, nil); err != nil {
			log.SysLogger.Errorf("call Service2.APIPrintParams failed, err:%v", err)
		}

		// 模拟有入参,但是不传
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIPrintParams", nil, nil); err != nil {
			log.SysLogger.Errorf("call Service2.APIPrintParams failed, err:%v", err)
		}
	})
	s.AfterFunc(time.Second*4, "method test demo3", func(timer *timingwheel.Timer, args ...interface{}) {
		...
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIPrintIndefiniteParams", []interface{}{1, "2", abc{1, 2}, "ddddd"}, nil); err != nil {
	...
	s.AfterFunc(time.Second*5, "method test demo4", func(timer *timingwheel.Timer, args ...interface{}) {
		...
		var out int
		var out2 string
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIMultiRet", nil, []interface{}{&out, &out2}); err != nil {
	...
	s.AfterFunc(time.Second*6, "method test demo5", func(timer *timingwheel.Timer, args ...interface{}) {
		...
		if err := s.SelectSameServer("", "Service2").Send(ctx, "APICallback", nil); err != nil {
	...
	s.AfterFunc(time.Second*2, "rpc test demo", func(timer *timingwheel.Timer, args ...interface{}) {
		if err := s.SelectSameServer("1", "Service3").Call(ctx, "RPCTest2", nil, nil); err != nil {
			log.SysLogger.Errorf("call Service3.RPCTest2 failed, err:%v", err)
		}
		ctxWithTimeout, cancel := context.WithTimeout(ctx, time.Second*5)
		defer cancel()
		if err := s.SelectSameServer("2", "Service3").Call(ctxWithTimeout, "RPCTest2", nil, nil); err != nil {
			log.SysLogger.Errorf("call Service3.RPCTest2 failed, err:%v", err)
		}
		if err := s.SelectSameServer("1", "Service3").Send(ctx, "RPCTest2", nil); err != nil {
			log.SysLogger.Errorf("call Service3.RPCTest2 failed, err:%v", err)
		}
	})
	s.AfterFunc(time.Second*2, "rpc test demo1", func(timer *timingwheel.Timer, args ...interface{}) {
		out := &msg.Msg_Test_Resp{}
		if err := s.SelectSameServer("1", "Service3").Call(ctx, "RPCSum", &msg.Msg_Test_Req{A: 1, B: 2}, out); err != nil {
			log.SysLogger.Errorf("call Service3.RPCSum failed, err:%v", err)
		}
		if err := s.SelectSameServer("2", "Service3").Send(ctx, "RPCSum", &msg.Msg_Test_Req{A: 1, B: 3}); err != nil {
			log.SysLogger.Errorf("send Service3.RPCSum failed, err:%v", err)
		}
		if _, err := s.SelectSameServer("1", "Service3").AsyncCall(ctx, "RPCSum", &msg.Msg_Test_Req{A: 1, B: 2}, &dto.AsyncCallParams{
		...
		// 测试调用对象返回错误
		if err := s.SelectSameServer("2", "Service3").Call(ctx, "RPCTestWithError", &msg.Msg_Test_Req{A: 1, B: 2}, nil); err != nil {
		...
		defer cancel()
		if err := s.SelectSameServer("1", "Service3").Call(ctxWithTimeout, "RpcTestWithError", nil, nil); err != nil {
			log.SysLogger.Errorf("call Service3.RpcTestWithError failed, err:%v", err)
		}
	})
	...
	s.AfterFunc(time.Second, "test2", func(timer *timingwheel.Timer, args ...interface{}) {
		var out int
		// 1. 固定参数->全部参数 (正常调用)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithFixedParams", []interface{}{1, 2}, &out); err != nil {
		...
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithFixedParams", []interface{}{4}, &out); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithFixedParams failed, err:%v", err)
		}
		log.SysLogger.Debugf("call Service2.APIWithFixedParams out2:%d", out)

		// 3. 固定参数->没有参数 (返回参数数量错误)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithFixedParams", nil, &out); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithFixedParams failed, err:%v", err)
		}
		log.SysLogger.Debugf("call Service2.APIWithFixedParams out3:%d", out)

		// 4. 固定参数->全部参数,不接收返回值 (正常调用,out依然输出3)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithFixedParams", []interface{}{5, 6}, nil); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithFixedParams failed, err:%v", err)
		}
		log.SysLogger.Debugf("call Service2.APIWithFixedParams out4:%d", out)

		//5. 不固定参数->有固定参数和不定参和返回值 给出全部参数 (正常调用)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithFixedAndIndefiniteParams", []interface{}{7, 8, "9", "10"}, &out); err != nil {
		...
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithFixedAndIndefiniteParams", []interface{}{11, 12}, &out); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithFixedAndIndefiniteParams failed, err:%v", err)
		}
		...
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithFixedAndIndefiniteParams", []interface{}{13}, &out); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithFixedAndIndefiniteParams failed, err:%v", err)
		}
		log.SysLogger.Debugf("call Service2.APIWithFixedAndIndefiniteParams out7:%d", out)

		// 8.  不固定参数->有固定参数和不定参和返回值 不给任何参数 (返回参数数量错误)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithFixedAndIndefiniteParams", nil, &out); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithFixedAndIndefiniteParams failed, err:%v", err)
		}
		...
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithFixedAndIndefiniteParams", []interface{}{14, 15, "16", "17"}, nil); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithFixedAndIndefiniteParams failed, err:%v", err)
		}
		log.SysLogger.Debugf("call Service2.APIWithFixedAndIndefiniteParams out8:%d", out)

		//9. 没有参数-> 有返回值 (正常调用)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithoutParams", nil, &out); err != nil {
		...
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithoutParams", 1, &out); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithoutParams failed, err:%v", err)
		}
		log.SysLogger.Debugf("call Service2.APIWithoutParams out2:%d", out)

		// 11. 没有参数-> 没有返回值 (正常调用)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithoutParams", nil, nil); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithoutParams failed, err:%v", err)
		}
		...
		//12. 多返回值 -> 全部接收 (正常调用)
		var out1, out2 int
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithMultiResults", nil, []interface{}{&out1, &out2}); err != nil {
		...
		// 13. 多返回值 -> 部分接收 (正常调用)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithMultiResults", nil, []interface{}{&out2}); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithMultiResults failed, err:%v", err)
		}
		// 输出 1, 1
		...
		// 14. 多返回值 -> 全都不接收 (正常调用)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithMultiResults", nil, nil); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithMultiResults failed, err:%v", err)
		}
		// 输出 1, 1
		...
		// 15. 多返回值 -> 使用错误类型接收 (报错)
		if err := s.SelectSameServer("", "Service2").Call(ctx, "APIWithMultiResults", nil, &out1); err != nil {
			log.SysLogger.Errorf("call Service2.APIWithMultiResults failed, err:%v", err)
		}
		// 输出 1, 1
...
func (s *Service2) APICallback() {
	...
	if err := s.SelectSameServer("", "Service1").Send(context.Background(), "APITest1", nil); err != nil {
