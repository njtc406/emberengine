...
func (s *Service1) OnInit() error {
	...
	//		s.GetLogger().Errorf("call Service2.APIPrintParams failed, err:%v", err)
	//	}
	...
	//		s.GetLogger().Errorf("call Service2.APIPrintIndefiniteParams failed, err:%v", err)
	//	}
	...
	//		s.GetLogger().Errorf("call Service2.APIMultiRet failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================99999999999")
	//	s.GetLogger().Debugf("call Service2.APIMultiRet out:%d, out2:%s", out, out2)
	//	return nil
	//})
	//
	...
	//		s.GetLogger().Errorf("call Service2.APICallback failed, err:%v", err)
	//	}
	//
	...
	//		s.GetLogger().Errorf("call Service3.RPCSum failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================12")
	//	if err := s.Select(rpc.WithName(ServiceNameTest3), rpc.WithSid("2")).Send(ctx, "RPCSum", &msg.Msg_Test_Req{A: 1, B: 3}); err != nil {
	//		s.GetLogger().Errorf("send Service3.RPCSum failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================13")
	//	if _, err := s.Select(rpc.WithName(ServiceNameTest3), rpc.WithSid("1")).AsyncCall(ctx, "RPCSum", &msg.Msg_Test_Req{A: 1, B: 2}, &dto.AsyncCallParams{
	//		Params: []interface{}{1, 2, "a"},
	//	}, func(ctx context.Context, data interface{}, err error, params ...interface{}) {
	//		if err != nil {
	//			s.GetLogger().Errorf("AsyncCall Service3.RPCSum response failed, err:%v", err)
	//			return
	//		}
	//
	//		s.GetLogger().Debugf("AsyncCall Service3.RPCSum params:%+v", params)
	//
	//		resp := data.(*msg.Msg_Test_Resp)
	//		s.GetLogger().Debugf("AsyncCall Service3.RPCSum out:%d", resp.Ret)
	//	}); err != nil {
	//		s.GetLogger().Errorf("AsyncCall Service3.RPCSum failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================14")
	//
	//	// 测试调用对象返回错误
	//	if err := s.Select(rpc.WithName(ServiceNameTest3), rpc.WithSid("1")).Call(ctx, "RPCTestWithError", &msg.Msg_Test_Req{A: 1, B: 2}, nil); err != nil {
	//		s.GetLogger().Errorf("call Service3.RPCTestWithError failed, err:%v", err)
	//	}
	...
	//	defer cancel()
	//	if err := s.Select(rpc.WithName(ServiceNameTest3), rpc.WithSid("1")).Call(ctxWithTimeout, "RpcTestWithError", nil, nil); err != nil {
	//		s.GetLogger().Errorf("call Service3.RpcTestWithError failed, err:%v", err)
	//	}
	...
	//
	////cast test
	//s.AfterFunc(time.Second*1, "cast test", func(ctx context.Context, timer *timingwheel.Timer, args ...interface{}) error {
	//	s.GetLogger().Debugf("================================>>>")
	...
	//		s.GetLogger().Errorf("call Service3.RPCSum failed, err:%v", err)
	//	} else {
	//		s.GetLogger().Debugf("call Service3.RPCSum result:%v", resp)
	//	}
	//
	...
	// other test
	//s.AfterFunc(time.Second*7, "other test", func(ctx context.Context, timer *timingwheel.Timer, args ...interface{}) error {
	//	// TODO 测试各种类型的筛选器
	//	return nil
	//})

	// 测试各种参数情况
	//s.AfterFunc(time.Second*10, "test2", func(ctx context.Context, timer *timingwheel.Timer, args ...interface{}) error {
	//	var out int
	//	// 1. 固定参数->全部参数 (正常调用)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithFixedParams", []interface{}{1, 2}, &out); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithFixedParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================19")
	//	s.GetLogger().Debugf("call Service2.APIWithFixedParams out1:%d", out)
	//
	//	// 2. 固定参数->部分参数 (返回参数数量错误)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithFixedParams", []interface{}{4}, &out); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithFixedParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================20")
	//	s.GetLogger().Debugf("call Service2.APIWithFixedParams out2:%d", out)
	//
	//	// 3. 固定参数->没有参数 (返回参数数量错误)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithFixedParams", nil, &out); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithFixedParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================21")
	//	s.GetLogger().Debugf("call Service2.APIWithFixedParams out3:%d", out)
	//
	//	// 4. 固定参数->全部参数,不接收返回值 (正常调用,out依然输出3)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithFixedParams", []interface{}{5, 6}, nil); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithFixedParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================22")
	//	s.GetLogger().Debugf("call Service2.APIWithFixedParams out4:%d", out)
	//
	//	//5. 不固定参数->有固定参数和不定参和返回值 给出全部参数 (正常调用)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithFixedAndIndefiniteParams", []interface{}{7, 8, "9", "10"}, &out); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithFixedAndIndefiniteParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================23")
	//	s.GetLogger().Debugf("call Service2.APIWithFixedAndIndefiniteParams out5:%d", out)
	//
	//	// 6. 不固定参数->有固定参数和不定参和返回值 给出全部固定部分,不给变参部分 (正常调用)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithFixedAndIndefiniteParams", []interface{}{11, 12}, &out); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithFixedAndIndefiniteParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================24")
	//	s.GetLogger().Debugf("call Service2.APIWithFixedAndIndefiniteParams out6:%d", out)
	//
	//	// 7. 不固定参数->有固定参数和不定参和返回值 给出部分固定部分,不给变参部分 (返回参数数量错误)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithFixedAndIndefiniteParams", []interface{}{13}, &out); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithFixedAndIndefiniteParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================25")
	//	s.GetLogger().Debugf("call Service2.APIWithFixedAndIndefiniteParams out7:%d", out)
	//
	//	// 8.  不固定参数->有固定参数和不定参和返回值 不给任何参数 (返回参数数量错误)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithFixedAndIndefiniteParams", nil, &out); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithFixedAndIndefiniteParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================26")
	//	s.GetLogger().Debugf("call Service2.APIWithFixedAndIndefiniteParams out8:%d", out)
	//
	//	// 9. 不固定参数->有固定参数和不定参和返回值 给出全部参数,不接收返回值 (正常调用,out依然输出23)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithFixedAndIndefiniteParams", []interface{}{14, 15, "16", "17"}, nil); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithFixedAndIndefiniteParams failed, err:%v", err)
	//	}
	...
	//
	//	//9. 没有参数-> 有返回值 (正常调用)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithoutParams", nil, &out); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithoutParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================28")
	//	s.GetLogger().Debugf("call Service2.APIWithoutParams out:%d", out)
	//
	//	// 10. 没有参数-> 给一个参数 (返回参数数量错误)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithoutParams", 1, &out); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithoutParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================29")
	//	s.GetLogger().Debugf("call Service2.APIWithoutParams out2:%d", out)
	//
	//	// 11. 没有参数-> 没有返回值 (正常调用)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithoutParams", nil, nil); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithoutParams failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================30")
	//	s.GetLogger().Debugf("call Service2.APIWithoutParams out3:%d", out)
	//
	//	//12. 多返回值 -> 全部接收 (正常调用)
	//	var out1, out2 int
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithMultiResults", nil, []interface{}{&out1, &out2}); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithMultiResults failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================31")
	//	// 输出 1, 2
	//	s.GetLogger().Debugf("call Service2.APIWithMultiResults 1 out1:%d, out2:%d", out1, out2)
	//
	//	// 13. 多返回值 -> 部分接收 (正常调用)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithMultiResults", nil, []interface{}{&out2}); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithMultiResults failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================32")
	//	// 输出 1, 1
	//	s.GetLogger().Debugf("call Service2.APIWithMultiResults 2 out1:%d, out2:%d", out1, out2)
	//
	//	// 14. 多返回值 -> 全都不接收 (正常调用)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithMultiResults", nil, nil); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithMultiResults failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================33")
	//	// 输出 1, 1
	//	s.GetLogger().Debugf("call Service2.APIWithMultiResults 3 out1:%d, out2:%d", out1, out2)
	//
	//	// 15. 多返回值 -> 使用错误类型接收 (报错)
	//	if err := s.Select(rpc.WithName(ServiceNameTest2)).Call(ctx, "APIWithMultiResults", nil, &out1); err != nil {
	//		s.GetLogger().Errorf("call Service2.APIWithMultiResults failed, err:%v", err)
	//	}
	//	s.GetLogger().Debugf("==========================================34")
	//	// 输出 1, 1
	//	s.GetLogger().Debugf("call Service2.APIWithMultiResults 4 out1:%d, out2:%d", out1, out2)
	//	return nil
	//})
...
func (s *Service1) APITest1() {
	s.GetLogger().Debugf("call %s func APITest1", s.GetName())
}
