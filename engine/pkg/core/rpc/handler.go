...
func compileCallFunc(methodInfo *def.MethodInfo) func(req interface{}) (interface{}, error) {
	return func(req interface{}) (interface{}, error) {
		...
		if len(params) != len(methodInfo.In) {
			log.SysLogger.Errorf("method[%s] param count not match, need: %d  got: %d", methodInfo.Method.Name, len(methodInfo.In), len(params))
			return nil, def.InputParamNotMatch
		}
		...
		if len(results) != len(methodInfo.Out) {
			// 这里应该不会触发,因为参数检查的时候已经做过了
			log.SysLogger.Errorf("method[%s] return value count not match", methodInfo.Method.Name)
			return nil, def.OutputParamNotMatch
		}
