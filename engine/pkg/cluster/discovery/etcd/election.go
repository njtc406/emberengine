...
func (e *etcdMasterElection) TryAcquireMaster(ctx context.Context, masterKey, group string, leaseRef disc.LeaseRef) (bool, int64, error) {
	id, ok := leaseRef.(clientv3.LeaseID)
	if !ok || !isEtcdClientConnected(e.d.client) {
		return false, 0, fmt.Errorf("etcd client not connected or invalid leaseRef")
	}
	...
	// Use the global revision of the successful txn as a monotonic fencing token.
